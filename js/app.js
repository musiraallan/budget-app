...
class HTML{
     ...
     addExpenseToList(name, amount) {
          ...
          li.innerHTML = `
               ${name}
               <span class="badge badge-primary badge-pill">Kshs. ${amount}</span>
          `;
     ...
     trackBudget(amount) {
          const budgetLeftShillings = budget.subtractFromBudget(amount);
          budgetLeft.innerHTML = `${budgetLeftShillings}`;

          // Check when 25% is left
          if( (budget.budget / 4 ) > budgetLeftShillings ){
               budgetLeft.parentElement.parentElement.classList.remove('alert-success', 'alert-warning');
               budgetLeft.parentElement.parentElement.classList.add('alert-danger');

          } else if( (budget.budget / 2 ) > budgetLeftShillings  ) {
...
function eventListeners() {
     ...
     document.addEventListener('DOMContentLoaded', function() {
          ...
          userBudget = prompt(' Enter your budget with no commas');
